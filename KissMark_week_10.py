...
# Ref: PyCryptodome - RSA
#                   - https://pycryptodome.readthedocs.io/en/latest/src/public_key/rsa.html
# Ref: cryptobook - RSA Encrypt / Decrypt - Examples
...
def RSA_padding():
    ...
    for i in range(len(ciphers)):
        try:
            ...
            base64_to_bytes = base64.b64decode(ciphers[i])
            decrypted = decryptor.decrypt(base64_to_bytes)
            print('Decrypted:', decrypted)
        except ValueError:
            print("This is not the right key to decrypt this message.")


RSA_padding()
# Solution: I love cryptography still


# Ref: cryptobook - RSA: Sign / Verify - Examples
#                 - https://cryptobook.nakov.com/digital-signatures/rsa-sign-verify-examples
def Signature():

    f = open('my_friend_key.pub', 'r')
    frinds_key = RSA.import_key(f.read())

    # RSA verify signature
    for x in range(len(messages)):
        hash = int.from_bytes(sha512(messages[x]['text']).digest(), byteorder='big')
        hashFromSignature = pow(int.from_bytes(base64.b64decode(messages[x]['signature']), byteorder='big'), frinds_key.e, frinds_key.n)
